...
\emph{Crucially, no clean token- or window-level labels are provided}:
the primary supervision signal is restricted to the sequence level. For
long-form responses we additionally have access to a noisy auxiliary
signal: an atomic-fact verification pipeline yields per-atom spans
$\{(\ell_m, r_m, \tilde{y}_m)\}$ over $\vec{\hat{g}}_i$, where each
$\tilde{y}_m \in \{0, 1\}$ is broadcast from the fact-level
verification result. As we discuss in
Section~\ref{sec:non_static}, these per-atom labels are coarse -- spans
typically contain tokens that are not themselves hallucinated -- so we
treat them as auxiliary supervision rather than ground-truth
window-level labels.
...
per-window scores must be learned under \emph{weak supervision}: from
$y$ alone, and -- when long-form atomic-fact pipelines are available --
also from the noisy per-atom auxiliary labels $\tilde{y}_m$ described
above. Beyond the in-domain setting, we additionally consider
