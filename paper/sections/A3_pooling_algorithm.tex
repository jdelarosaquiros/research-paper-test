...
Our pooling algorithm is presented in Algorithm~1 below.

\begin{figure}[h]
\centering
\fbox{\begin{minipage}{0.92\linewidth}
Algorithm 1: Pooling\\[2pt]
Require: Hidden-state activations $A \in \mathbb{R}^{L_M \times N \times D_M}$, target sizes $L_p, N_p$\\
Ensure: Pooled activations $A \in \mathbb{R}^{L_p \times N_p \times D_M}$\\
...
function and yet produce entirely different hidden-state activations. This discrepancy stems from internal
...
Recall that $A_l$ denotes the activations at layer $l \in [L]$. Let $(Q_l, K_l, V_l)$ denote the query, key, and
value matrices at layer $l$, computed as:
\begin{equation}
(Q_l, K_l, V_l) = (A_{l-1} W_l^{Q}, A_{l-1} W_l^{K}, A_{l-1} W_l^{V}).
\end{equation}
...
W_{l+1}^{X} &\to P\, W_{l+1}^{X} \quad \text{for } X \in \{Q, K, V\}.
\end{align}
...
Equation~\eqref{eq:ffn}, the hidden-state activations differ significantly, as the transformation permutes the feature
