...
The Encoder Only row aggregates per-window logits to a
single number per response via top-$k$ MIL, matching the
training-time aggregator (Section~\ref{sec:temporal_head}); the
full Temp-ViT row uses the Stage~3 passage-level head.

Encoder Only wins WMT19 ($0.790$) and lands within $0.006$
