...
\paragraph{Baselines.} We organize baselines into three groups that
mirror the row bands of Tables~\ref{tab:main}--\ref{tab:claim} and
Table~\ref{tab:polygraph}. All baselines are run through the
...
re-applied per claim, per sentence, or per token when needed.

\emph{Confidence} baselines score uncertainty from the model's
next-token distribution over generated tokens:
\textbf{Maximum Sequence Probability} (the LM-Polygraph default),
\textbf{Perplexity} \citep{fomicheva2020unsupervised},
\textbf{Mean Token Entropy} \citep{fomicheva2020unsupervised},
\textbf{Self-Certainty} \citep{kang2025selfcertainty},
\textbf{TokenSAR} \citep{duan2023},
\textbf{Mean Pointwise Mutual Information} \citep{takayama2019pmi},
and \textbf{BoostedProbSequence} \citep{dinh2025boostedprob}.

\emph{Mechanistic} baselines read model internals (hidden states or
attention patterns) instead of relying solely on the output
distribution: \textbf{Contextualized Sequence Likelihood (CSL)}
\citep{lin2024csl},
\textbf{Attention Score} \citep{sriramanan2024attention},
\textbf{Recurrent Attention-based Uncertainty Quantification (RAUQ)}
\citep{vazhentsev2025rauq},
\textbf{Fact-Probe} \citep{han2025factprobe},
\textbf{Feature-Gaps} \citep{bakman2025featuregaps}, and
\textbf{Act-ViT} \citep{barshalom2025}, the pooled-tensor sequence
encoder our architecture builds on.

\emph{Ours} reports Temp-ViT in two variants:
\textbf{Temp-ViT~(Encoder~Only)} -- the Stage-2 detector
(Section~\ref{sec:temporal_head}) used as a stand-alone per-window
scorer aggregated by top-$k$ MIL, which is the natural comparison for
Table~\ref{tab:polygraph}; and \textbf{Temp-ViT} -- the full
three-stage pipeline (Section~\ref{sec:stage3}) whose per-atom output
is reported in Tables~\ref{tab:main}--\ref{tab:claim}.
