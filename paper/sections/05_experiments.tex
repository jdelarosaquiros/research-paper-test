...
that the literature treats separately. The first setting is sentence-level
...
LongFact \citep{wei2024}. The second setting is sequence-level uncertainty on
...
generations; and LongFact \citep{wei2024}, long-form
...
mirror the row bands of Tables~\ref{tab:main} and \ref{tab:claim} and
...
Confidence baselines score uncertainty from the model's
...
and BoostedProbSequence \citep{dinh2025boostedprob}.

Mechanistic baselines read model internals (hidden states or
attention patterns) instead of relying solely on the output
distribution: Contextualized Sequence Likelihood (CSL)
\citep{lin2024csl},
...
Ours reports Temp-ViT in two variants:
Temp-ViT~(Encoder~Only), the Stage-2 detector
...
Table~\ref{tab:polygraph}; and Temp-ViT, the full
three-stage pipeline (Section~\ref{sec:stage3}) whose per-claim output
is reported in Tables~\ref{tab:main} and \ref{tab:claim}.
