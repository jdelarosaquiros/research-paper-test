...
We introduce Temp-ViT, a three-stage temporal uncertainty localization
framework that closes the long-form / real-time gap for mechanistic
hallucination detection. The encoder reads the full activation tensor;
the Mamba-2 temporal head produces per-token logits under weakly
supervised top-$k$ MIL on per-atom segments; the passage-level
refinement step turns those logits into per-atom scores that
discriminate uncertain atoms from confident ones within the same
response. On RAGTruth, FactScore, and LongFact across three
instruction-tuned LLMs, the per-token output reaches sentence- and
claim-level AUROC competitive with pooled tensor baselines while
running $\approx 25\times$ faster on long passages. On the
...
noisy: atomic-fact pipelines mis-attribute hallucinated tokens to
neighbouring supported tokens, and the relevant activation signal can
fall outside the span (Figure~\ref{fig:signal_propagation}). The
...
\paragraph{Acknowledgements.} G.B. is supported by the Jacobs Qualcomm
PhD Fellowship. F.F. conducted this work supported by an Aly Kaufman
Post-Doctoral Fellowship. H.M. is a Robert J. Shillman Fellow and is
supported by the Israel Science Foundation through a personal grant
(ISF 264/23) and an equipment grant (ISF 532/23).
