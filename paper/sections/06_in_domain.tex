...
(any unsupported claim in a sentence flips the sentence to
...
claim). Temp-ViT denotes the full three-stage pipeline; at each
...
claims against each other within the same passage.
Table~\ref{tab:long_form_stats} reports the passage- and claim-length
distributions that determine which granularity is feasible to
supervise; LongFact and FactScore claims are typically much shorter
...
($(L_p, N_p) = (4, 20)$) preserves most of the per-claim AUROC while
