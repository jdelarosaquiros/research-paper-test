...
ill-suited to real-time long-form generation: full-activation probes
face a speed--accuracy tradeoff, where keeping the activations small
and pooled loses accuracy as responses grow long, while scaling the
representation up sacrifices inference speed by requiring one forward
per token. Even when a full-activation method does scale, a single
...
\paragraph{Full-activation probes do not scale to long responses.}
...
compressing the whole response into one fixed activation image,
Temp-ViT scores local token windows and models their temporal
evolution.
